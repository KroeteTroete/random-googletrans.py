...
def breakTranslation():
    ...
    while inloop == True:
        withlist = str(input(
            "Do you want the languages to be chosen randomly or define a list of languages?\n1: random\n2: list\n"))
        if withlist == "1":
            withlist = False
            inloop = False
        elif withlist == "2":
    ...
    if withlist == True:
        answer = input(
            "Enter the list of languages. Each ISO-639-1 Code should be seperated by ';'\nFor example: en;de;es;fr;la\nThe Language the text is in will be appended automatically\n")
        listOfLangs = answer.split(";")

    inloop = True
    while inloop == True:
        retranslation = str(input(
            "Do you want to enable retranslation? (The retranslation will not be used in the translations after it)\n1: Yes\n2: No\n"))
        if retranslation == "1":
            retranslation = True
            inloop = False
